...
SELECT DISTINCT s.customer_id,
	   f.min_date, s.product_id, m.product_name
	 FROM sales AS s
...
-- QUESTION 10: Within the first week after a customer joins the program (including their join date) 
-- they earn 2x points on all items, not just sushi - how many points do customer A and B have at the end of January?

WITH cte AS (
	SELECT s.customer_id, s.order_date, m.join_date, m.join_date+INTERVAL '7 DAY' AS week_after_joining, me.product_name, me.price
	FROM sales AS s
		 INNER JOIN menu AS me
		 ON me.product_id = s.product_id
		 INNER JOIN members AS m
		 ON m.customer_id = s.customer_id)
SELECT customer_id, SUM(CASE WHEN order_date BETWEEN join_date AND week_after_joining THEN 2 * 10 * price
		    				 WHEN order_date NOT BETWEEN join_date AND week_after_joining AND product_name = 'sushi' THEN 2 * 10 * price
            				 WHEN order_date NOT BETWEEN join_date AND week_after_joining AND product_name != 'sushi' THEN 10 * price
            				 END) AS total_points
FROM cte
